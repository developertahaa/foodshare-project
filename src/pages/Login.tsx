...
export default function LoginSignup() {
  ...
  const [formData, setFormData] = useState({
    ...
    password: '',
    name: '',
    phone: '', // Added phone number field to form data
  })
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const url = isLogin ? 'http://localhost:5003/login' : 'http://localhost:5003/signup'
    ...
    setFormData({
      ...
      password: '',
      name: '',
      phone: '', // Reset phone number after form submission
    })
  }
        ...
        <div className="bg-white py-8 px-4 shadow sm:rounded-lg sm:px-10">
          <form className="space-y-6" onSubmit={handleSubmit}>
            ...
            <div>
              ...
              {!isLogin && passwordStrength < 3 && formData.password && (
                <p className="mt-2 text-sm text-red-600" id="password-error">
                  <AlertCircle className="h-5 w-5 text-red-500 inline mr-1" />
                  Password is too weak. Try adding a mix of letters, numbers, and special characters.
                </p>
              )}
            </div>
